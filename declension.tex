...
語基（\ref{np:86}條參照）に準じて變化する部分を有す。單具 akṣṇā 等
（\ref{np:87}條）。
\end{enumerate}
...
\numberParagraph
複母音語基，nau （舟），go （牛）。\endnote{底本では両{数}形について主・呼・業ではなく
  呼・業が \} でまとめられている（主格の欄は{空}欄）。また具・為をまとめる \} が
  複{数}形ではなく{単}{数}形の列に付いている。}
\begin{center}
