...
\eg{
  adya api = adyāpi （今日も亦）\mymp{adya = 今日，api = また} \\
  yadi icchasi = yadīcchash （若しも汝が欲するならば）\mymp{yadi = もし，icchasi = 欲す \rt iṣ 現在語基 iccha 二単} \\
  sādhu uktam = sādhūktam （よくも云はれた） 。\mymp{sādhu = よく，uktam \label{mp:uktam} = 言う \rt vac 過去受動分詞 ukta- 単業}}

\numberParagraph
a, ā は等しからざる單母音と共に重韻となり e, o, ai, au
と共に複重韻となる。
\eg{
  ca iti = ceti （而して以上）\mymp{ca = そして，iti = 以上} \\
  tena uktam = tenoktam （彼は云へり）\mymp{tena = tad- 男/中単具，uktam = \pageref{mp:uktam}}\\
  adhunā ṛṣir yajate = adhunarṣir yajate （今聖仙は
  祭祀す）\endnote{底本では「祀」は「{\HanazonoA 祀󠄄}」。}\mymp{adhunā = 今，ṛṣir = } \\
...
\eg{
  ...
  tayos ekaḥ = tayor ekaḥ （二人のうちの一人）\\
...
語の終の ās は一切軟音の前にその s を消失す。
\eg{
  janās āgacchanti = janā āgacchanti （人々は來れり）}
...
\eg{
  dhanus + aḥ = dhanuṣaḥ （弓）屬單 \\
  dhanus = dhanūṃṣi （複主）\\
  vac + syati = (vak + syati =) vakṣyati （彼は語るべし）\\
  sarpis = sarpiṣā （淨酪を以て）具單
\item[されど] sarpiḥ （語の終）\\
  manasā （a に先立たる）\\
  tamisram （r 從ふ）。}
